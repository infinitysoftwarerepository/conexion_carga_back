...
def list_my_loads(
    status: str = Query("all", pattern="^(all|published|expired)$"),
    skip: int = 0,
    limit: int = 100,
    db: Session = Depends(get_db),
    current: models.User = Depends(get_current_user),
...
@router.put("/{load_id}", response_model=schemas.CargoOut)
def update_and_republish(
    load_id: str,
    payload: schemas.CargoUpdate,
    db: Session = Depends(get_db),
    current: models.User = Depends(get_current_user),
):
    obj = crud.reactivate_cargo(db, load_id, owner_id=current.id, data=payload)
    if not obj:
        raise HTTPException(status_code=404, detail="No encontrado o sin permisos.")
    return obj
